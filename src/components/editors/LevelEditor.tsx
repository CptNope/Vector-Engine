            ...
            <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900">
              <div className="flex items-center gap-4">
                <input 
                  ...
                  onChange={e => updateLevel({ ...selectedLevel, name: e.target.value })}
                ...
                <select 
                  ...
                  onChange={e => updateLevel({ ...selectedLevel, scrollDirection: e.target.value as any })}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input 
                    ...
                    onChange={e => updateLevel({ ...selectedLevel, length: Number(e.target.value) })}
                    className="w-20 bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-sm text-zinc-300"
                  />
                </div>
              </div>
              <button 
                onClick={() => { deleteLevel(selectedLevel.id); setSelectedId(null); }}
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-48 bg-zinc-900 border-r border-zinc-800 p-4 flex flex-col gap-4">
                <div className="space-y-2">
                  <h3 className="text-xs font-semibold text-zinc-500 uppercase">Tools</h3>
                  <button 
                ...
                <div className="space-y-2 mt-auto border-t border-zinc-800 pt-4">
                  <h3 className="text-xs font-semibold text-zinc-500 uppercase">Level Transitions</h3>
                  <div className="space-y-1">
                    <label className="text-xs text-zinc-400">Next Level</label>
                    <select 
                      value={selectedLevel.nextLevelId || ''}
                      onChange={e => updateLevel({ ...selectedLevel, nextLevelId: e.target.value || null })}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300"
                    >
                      <option value="">None</option>
                      {gameData.levels.filter(l => l.id !== selectedLevel.id).map(l => (
                        <option key={l.id} value={l.id}>{l.name}</option>
                      ))}
                    </select>
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs text-zinc-400">Next Story Node</label>
                    <select 
                      value={selectedLevel.nextStoryNodeId || ''}
                      onChange={e => updateLevel({ ...selectedLevel, nextStoryNodeId: e.target.value || null })}
                      className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-300"
                    >
                      <option value="">None</option>
              ...
              <div className="flex-1 overflow-auto bg-zinc-950 p-8 flex justify-center">
                <div className="relative shadow-2xl shadow-black">
                  <canvas 
                    ref={canvasRef}
                    width={selectedLevel.scrollDirection === 'vertical' ? 800 : selectedLevel.length}
                    height={selectedLevel.scrollDirection === 'vertical' ? selectedLevel.length : 600}
                    onClick={handleCanvasClick}
                    className="cursor-crosshair"
                  />
                  <div className="absolute top-2 left-2 text-xs text-zinc-500 pointer-events-none">
                    {selectedLevel.scrollDirection === 'vertical' ? 'Start (Bottom) -> End (Top)' : 'Start (Left) -> End (Right)'}
                  </div>
                </div>
              </div>
            </div>
